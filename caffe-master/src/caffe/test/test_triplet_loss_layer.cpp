...
class TripletLossLayerTest : public MultiDeviceTest<TypeParam> {
 ...
 protected:
  TripletLossLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(40, 1, 1, 1)),
        blob_bottom_y_(new Blob<Dtype>(40, 1, 1, 1)),
...
TYPED_TEST(TripletLossLayerTest, TestForward) {
  ...
  const int num_triplets = 4;
  const int num_set = this->blob_bottom_data_->num()/num_triplets;
  const int channels = this->blob_bottom_data_->channels();
  Dtype loss(0);
  for (int i = 0; i < num_set; ++i) {
      Dtype dist_sq(0);
      for (int j = 0; j < channels; ++j) {
        Dtype diff_pos = this->blob_bottom_data_->cpu_data()[(num_triplets)*i*channels+j] -
          this->blob_bottom_data_->cpu_data()[((num_triplets)*i+1)*channels+j];
        dist_sq += diff_pos*diff_pos;
        Dtype diff_neg = this->blob_bottom_data_->cpu_data()[(num_triplets)*i*channels+j] -
          this->blob_bottom_data_->cpu_data()[((num_triplets)*i+2)*channels+j];
